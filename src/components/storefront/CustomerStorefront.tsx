          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {onlineProducts.map((prod) => {
              ...
              return (
                <div
                  key={prod.id}
                  className="bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-white/10 shadow-xl shadow-black/20 p-3.5 flex flex-col justify-between hover:border-emerald-500/40 transition-all group"
                >
                  <div>
                    <div className="relative aspect-square w-full rounded-2xl overflow-hidden bg-slate-950/60 mb-3 border border-white/10">
                      <img
                        src={prod.image}
                        alt={prod.name}
                        referrerPolicy="no-referrer"
                      ...
                      {prod.stock <= prod.minStockAlert && !isOutOfStock && (
                        <span className="absolute top-2 left-2 px-2 py-0.5 bg-amber-500/90 border border-amber-400/40 backdrop-blur-md text-white text-[9px] font-bold rounded-md shadow-xs">
                          {isBn ? 'সীমিত স্টক' : 'Few left'}
                        </span>
                      )}
                      {isOutOfStock && (
                        <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center text-rose-300 text-xs font-bold">
                          {t.pos.outOfStock}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between gap-2">
                    ...
                    {inCart ? (
                      <div className="flex items-center gap-1 bg-emerald-500/20 border border-emerald-500/30 rounded-xl p-0.5">
                        <button
                          onClick={() => updateCartQuantity(prod.id, inCart.quantity - 1)}
                          className="w-6 h-6 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center text-white shadow-2xs"
                        >
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="w-5 text-center font-bold font-mono text-xs text-emerald-300">
                          {inCart.quantity}
                        </span>
                        <button
                          onClick={() => updateCartQuantity(prod.id, inCart.quantity + 1)}
